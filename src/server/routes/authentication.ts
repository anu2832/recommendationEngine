...
export const handleAuthEvents = (socket: Socket) => {
    socket.on('authenticate', async (data) => {
        ...
        try {
            ...
            if ((results as any).length > 0) {
                ...
                socket.emit('auth_response', { success: false, message: 'Invalid credentials' });
            }
        } catch (err) {
            socket.emit('auth_response', { success: false, message: 'Database error' });
    ...
    socket.on('register', async (data) => {
        ...
        try {
            const connection = await pool.getConnection();
            await connection.execute('INSERT INTO user (username, password, role ) VALUES (?, ?, ?)', [name, employeeId, role]);
            connection.release();
            ...
            socket.emit('register_response', { success: false, message: 'Database error' });
            console.error('Database query error', err);
        }
    });
