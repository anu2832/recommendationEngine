...
socket.on('auth_response', (data: { success: boolean; message: string; role?: string; userID: string }) => {
    console.log(data)
   if (data.success) {
...
socket.on('register_response', (data: { success: boolean; message: string; role?: string }) => {
   if (data.success) {
...
socket.on('show_rollover_response', (data: { success: boolean; message: string; userId?: string }) => {
   ...
   if (data.success) {
       ...
       socket.emit('vote_for_menu', { success: true, data: { userId: data.userId } });
   } else {
...
socket.on('add_item_response', (data) => {
   if (data.success) {
       ...
       rl.question('Do you want to try again to add Item ? (yes/no): ', (answer) => {
           if (answer.toLowerCase() === 'yes') {
               addItem("admin");
           } else {
            adminMenu();
...
socket.on('update_item_response', (data) => {
    if (data.success) {
        ...
        rl.question('Do you want to update another item? (yes/no): ', (answer) => {
            if (answer.toLowerCase() === 'yes') {
                updateItem("admin");
            } else {
                adminMenu();
        ...
        rl.question('Do you want to try again to update Item ? (yes/no): ', (answer) => {
            if (answer.toLowerCase() === 'yes') {
                updateItem("admin");
            } else {
                adminMenu();
...
async function updateItem(role: string) {
   ...
   socket.once('check_item_exists_response', async (response) => {
       if (response.success && response.exists) {
           ...
           socket.emit('update_item', { itemId:id, name: newName, price: newPrice });
       } else {
...
socket.on('view_menu_response', (data) => {
   if (data.success) {
       ...
       data.menu.forEach((item: { itemId: any; itemName: any; price: any; availability: any; mealType: any; }) => {
           console.log(`ID: ${item.itemId}, Name: ${item.itemName}, Price: ${item.price}, Availability: ${item.availability}, Meal Time: ${item.mealType}`);
...
socket.on('view_feedbacks_response', (data) => {
 ...
 socket.on('view_rolledOut_Menu', (data: { success: boolean; rollout: any, userId: string }) => {
    if (data.success) {
